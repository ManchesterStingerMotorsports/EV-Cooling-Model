...
trackfile = 'Endurance.mat' ;
% trackfile = 'Skidpad.mat' ;
vehiclefile = 'VehicleTemp.mat';
...
% 
% % saving figure
% %savefig(simname+".fig") % CHANGED BY ME
...
% saving .mat file
%save(simname+".mat",'veh','tr','sim') % CHANGED BY ME
% HUD
%toc
...
function [] = export_report(veh,tr,sim,freq,logid)
    ...
    T = readmatrix(filename,delimitedTextImportOptions('DataLines',[20,Inf],VariableTypes='double'));
    writematrix(T, sim.sim_name.data+'.xlsx');
    delete(filename);
    %disp('Converted to .xlsx')
    %movefile(sim.sim_name.data+'.xlsx', '../Data/')
    %disp('Copied to Battery Sim')
    % HUD
...
filename = 'Data/Lapsim_settings.xlsx' ;
